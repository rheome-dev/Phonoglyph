...
- Only files matching these patterns are included: apps/web/src/lib/visualizer/effects/ParticleNetworkEffect.ts, apps/web/src/lib/visualizer/effects/MetaballsEffect.ts, apps/web/src/remotion/RayboxComposition.tsx, apps/web/src/lib/visualizer/core/VisualizerManager.ts
...
export class ParticleNetworkEffect implements VisualEffect {
  ...
  private getRandomSpawnPosition(index: number): THREE.Vector3 {
    // Use Remotion's deterministic random() function with seeded values
    // This ensures the same particle index always spawns at the same position
    const isRendering = getRemotionEnvironment().isRendering;
    
    if (isRendering) {
      // Deterministic: use Remotion's seeded random function
      const x = (random(`particle-x-${index}`) - 0.5) * 4;
      const y = (random(`particle-y-${index}`) - 0.5) * 4;
      return new THREE.Vector3(x, y, 0);
    } else {
      // Live editor: use Math.random() for smooth animation
  ...
  private spawnManualParticles(deltaTime: number) {
    ...
    // Use deterministic random for rendering, Math.random() for live editor
    const randomValue = isRendering ? random(`particle-spawn-${Math.floor(currentTime * 10)}`) : Math.random();
