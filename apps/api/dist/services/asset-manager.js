...
class AssetManager {
    ...
    async startUsageTracking(fileId, projectId, usageType, usageContext = {}) {
        const { data, error } = await this.supabase
            ...
            .single();
        if (error) {
            logger_1.logger.error('Error starting usage tracking:', error);
    ...
    async endUsageTracking(usageId) {
        const { error } = await this.supabase
            ...
            .eq('id', usageId);
        if (error) {
            logger_1.logger.error('Error ending usage tracking:', error);
            throw new server_1.TRPCError({
                code: 'INTERNAL_SERVER_ERROR',
    ...
    async updateFileUsageStatus(fileId, status) {
        const { error } = await this.supabase
            ...
            .eq('id', fileId);
        if (error) {
            logger_1.logger.error('Error updating file usage status:', error);
            throw new server_1.TRPCError({
                code: 'INTERNAL_SERVER_ERROR',
    ...
    async getAssetUsage(fileId, projectId) {
        ...
        if (error) {
            logger_1.logger.error('Error fetching asset usage:', error);
            throw new server_1.TRPCError({
                code: 'INTERNAL_SERVER_ERROR',
    ...
    async getFileTags(fileId) {
        ...
        const { data: relationships, error: relError } = await this.supabase
            ...
            .eq('file_id', fileId);
        if (relError) {
            logger_1.logger.error('Error fetching file tag relationships:', relError);
            throw new server_1.TRPCError({
                code: 'INTERNAL_SERVER_ERROR',
        ...
        const { data: tags, error: tagError } = await this.supabase
            ...
            .order('name');
        if (tagError) {
            logger_1.logger.error('Error fetching file tags:', tagError);
            throw new server_1.TRPCError({
                code: 'INTERNAL_SERVER_ERROR',
    ...
    async replaceAsset(oldFileId, newFileId, preserveMetadata = true) {
        ...
        const { error: updateError } = await this.supabase
            ...
            .eq('id', newFileId);
        if (updateError) {
            logger_1.logger.error('Error updating replacement file:', updateError);
            throw new server_1.TRPCError({
                code: 'INTERNAL_SERVER_ERROR',
        ...
        const { error: markError } = await this.supabase
            ...
            .eq('id', oldFileId);
        if (markError) {
            logger_1.logger.error('Error marking old file as replaced:', markError);
